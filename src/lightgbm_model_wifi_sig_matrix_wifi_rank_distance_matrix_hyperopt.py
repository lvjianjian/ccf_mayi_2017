...
def main_kfold_hyperopt(offline, kfold=5, mall_ids=-1):
    ...
    for _index, mall_id in enumerate(mall_ids):
        ...
        def objective(argsDict):
            params = {
                ...
                'bagging_freq': int(argsDict["bagging_freq"]),
                'verbose': 0,
                'num_class': num_class

            }
            _train = lgb.Dataset(train_matrix, y)
            his = lgb.cv(params,
                         _train,
                         int(argsDict["n_round"]),
                         metrics=["multi_error"])
            return np.mean(his["multi_error-mean"])

        space = {
            'n_round': hp.quniform("n_round", 20, 400, 20),
            ...
            'bagging_freq': hp.quniform("bagging_freq", 5, 50, 5),
        }
    ...
    path = "../result/offline/{}_f{}_hyperopt".format(model_name, "num_class_{}".format(scala))
    ...
    if not offline:
        ...
        path = "../result/online/{}_f{}_hyperopt".format(model_name, "num_class_{}".format(scala))
        save_result(result, path, None)


if __name__ == '__main__':
    # main(offline=False)
    main_kfold_hyperopt(offline=True, mall_ids=["m_2467"])  # mall_ids=["m_690", "m_7168", "m_1375", "m_4187", "m_1920", "m_2123"]

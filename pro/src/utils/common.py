...
def merge_behavior_shopinfo(behavior_data, shop_data):
    # merge the behavior_data and shop_info 
    train_info = pd.merge(behavior_data, shop_data, on="shop_id", how="left")
    return train_info
...
def get_wifi_cache(mall_id, root_path, default = -115):
    df = pd.read_csv(os.path.join(root_path, 
        "{}_rank.csv".format(mall_id)))
    train_index = scipy.load(os.path.join(root_path, 
                            "{}_{}_index.npy".format("train", mall_id)))
    test_index = scipy.load(os.path.join(root_path,
                            "{}_{}_index.npy".format("test", mall_id)))
    train_use_wifi = scipy.load(os.path.join(root_path, 
                                "{}_{}_use_wifi.npy".format("train", mall_id))
                                )
    test_use_wifi = scipy.load(os.path.join(root_path,
                                "{}_{}_use_wifi.npy".format("test", mall_id))
                                )
    train_matrix = scipy.load(os.path.join(root_path, 
                            "{}_{}_matrix.npy".format("train", mall_id)))[()].toarray()
    test_matrix = scipy.load(os.path.join(root_path, 
                            "{}_{}_matrix.npy".format("test", mall_id)))[()].toarray()
...
def do_wifi_cache(root_path):
    wifi_info2csv([load_train(), load_testA()], ["train", "test"], root_path)
